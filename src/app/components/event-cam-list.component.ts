    ...
    <div>
        ...
        <div *ngIf="warnings && warnings.length > 0" style="padding-bottom: 30px">
          <div *ngFor="let camera of warnings">
            ...
            <ng-template #show_info>
              <mat-card *ngIf="camera.cam_enabled" class="camlist-info my-card">
                Camera "{{camera.cam_name}}" info:<br/> {{getHumanReadableError(camera)}}
              </mat-card>
            </ng-template>
          </div>
        </div>
      ...
      <ng-template #liveview_content_multiple>
        <div *ngFor="let camera of cameras">
          <div *ngIf="camera.cam_enabled && camera.cam_last_error != 'info_payment'" style="padding-bottom: 10px">
            <button
              mat-raised-button
              color="accent"
              (click)="onLiveClicked(camera.cam_id)">{{camera.cam_name}} LIVE</button>
          </div>
        </div>
      </ng-template>
...
export class EventCamListComponent extends CamListSelectionComponent implements OnInit {
    ...
    private getCameraSettingsByCamId(camId: number): CameraSettings {
        for (let camera of this.cameras) {
            if (camera.cam_id == camId)
                return camera;
        }
        return null;
    }
